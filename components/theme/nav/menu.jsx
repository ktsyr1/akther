...
export default function Menu() {
  return (
    <div>
      <div id="menu" className="mt-[58px] pup w-[300px] ui none">
        {menuList.map((item) => {
          if (item.href) {
            return (
              <Link href={item.href} key={item.name}>
                <a className={`box row p-4 ${item.status ? "" : ""}`}>
            ...
            return (
              <div
                className={`box row p-4 ${item.status ? "" : ""}`}
                key={item.name}
              >
                <div className="box px-4">{item.icon}</div>
                <p>{item.name}</p>
...
let menuList = [
  ...
  // { icon: <IconNotification />, name: "الاشعارات", href: "/notification", status: false, },
